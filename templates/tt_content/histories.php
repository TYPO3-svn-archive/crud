...
if ($this->get('mode') == 'PROCESS') {
	...
	$request = $params;
	$histDateArray = $this->getHistoryDates();
	if(count($histDateArray) > 1) {
		...
		for($i = -1; $i < count($config['view.']['histories']); $i++) {
			if(($i == -1 && !isset($request['showhistory'])) || ($request['showhistory'] == $i && isset($request['showhistory'])))
			...
			if(($i == 0 && !isset($request['compareWith'])) || ($request['compareWith'] == $i && isset($request['compareWith'])))
		...
		//t3lib_div::debug($config['view.']['data']);
		$diffArray = $this->getHistoryDiff($request['compareWith'], $request['showhistory']);
		//t3lib_div::debug($diffArray);
