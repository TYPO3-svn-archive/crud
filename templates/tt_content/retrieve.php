...
if ($this->get('mode') == 'PROCESS') {
	...
	if(!isset($params[history])) {
		...
		if($_POST['ajaxTarget']!="thickbox") {
			...
			$visitCount = $this->getLogUserCount('retrieve');
			$updateCount = $this->getLogUserCount('update');
			if ($creator) {
				echo '%%%created%%% %%%on%%% ' . $this->getCreationDate() . ' %%%by%%% ' . $creator . "\n";
			}
			
			if (isset($visitCount)) {
			$lastVisitor = $this->getLastLogUser('retrieve');
			...
			if ($visitCount > 0) {
				...
				echo ' - %%%atlast%%% %%%on%%% ' . $this->getLastLogDate('retrieve') . 
					' %%%by%%% ' . $lastVistor . "<br/>\n";
			}
		}
		if (isset($updateCount)) {
			$lastUpdater = $this->getLastLogUser('update');
			...
			if ($updateCount > 0) {
				...
				echo ' - %%%atlast%%% %%%on%%% ' . $this->getLastLogDate('update') . 
					' %%%by%%% ' . $lastUpdater . "<br/>\n";
				if(count($config['view.']['histories']) > 0)
					echo '</a>';
			}
