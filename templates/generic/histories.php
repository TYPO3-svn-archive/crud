...
if ($this->get('mode') == 'PROCESS') {
	...
	if (count($histDateArray) > 1) {
		...
		echo 'Historieeinträge:' . $historyCount . '<br />Zeige Unterschiede zu Versionen: <br/>';
		...
		for ($i = -1; $i < count($config['view.']['histories']); $i++) {
			if ($i == $showHistory) {
				...
				echo '<tr><td><input class="showhistory" type="radio" name="' . $fieldName1 . '" value="' . $i . '" />Eintrag vom ' . $histDateArray[$i] . '</td>';
			}
			if ($i == $compareWith) {
				echo '<td><input class="comparewith" type="radio" name="' . $fieldName2 . '" value="' . $i . '" checked="checked" />Eintrag vom ' . $histDateArray[$i] . '</td></tr>';
			} else {
				echo '<td><input class="comparewith" type="radio" name="' . $fieldName2 . '" value="' . $i . '" />Eintrag vom ' . $histDateArray[$i] . '</td></tr>';
			}
		}
	...
	echo '<a href="' . $this->getUrl($params) . '">zurück zum Datensatz</a>';
}
?>